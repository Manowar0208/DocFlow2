...
// ==========================================
// 🚨 CONFIGURATION (SPECIFIC TO ADMISSION)
// ==========================================
const MY_DEPARTMENT = 'admission'; 
const MY_PORT = 4003; 

// --- ROLE-SPECIFIC PASSKEYS ---
const ROLE_KEYS = {
    'RADMISSION': 'REVIEWER',     // Unlocks Reviewer tools
    'AADMISSION': 'ADMIN'         // Unlocks Admin tools
};
    ...
    res.send(`
        <body style="font-family:sans-serif; display:flex; justify-content:center; align-items:center; height:100vh; background:#1a1a1a; color:white; margin:0;">
            <form action="/login" method="POST" style="background:#2d2d2d; padding:40px; border-radius:12px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); width:320px; text-align:center;">
                <h1 style="color:#1abc9c; margin-bottom:10px;">🏛️ DEPT PORTAL</h1>
                <p style="color:#aaa; font-size:14px; margin-bottom:25px;">Enter Admission Role Key</p>
                ...
                <button type="submit" style="padding:12px; background:#1abc9c; color:white; border:none; border-radius:6px; cursor:pointer; width:100%; font-weight:bold; font-size:16px;">
                    ACCESS PORTAL
                </button>
            </form>
        </body>
...
app.post('/login', (req, res) => {
    ...
    res.send("<script>alert('Invalid Admission Key'); window.location='/login';</script>");
});
...
app.get('/', (req, res) => {
    ...
    <html>
        ...
        <body>
            <div class="nav">
                <div>
                    <h2 style="margin:0;">🏛️ ${MY_DEPARTMENT.toUpperCase()} PORTAL</h2>
            ...
            <div class="container">
                <div class="col ${role === 'REVIEWER' ? '' : 'locked'}">
                    ...
                    ${reviewQueue.length === 0 ? '<p>No admission apps waiting.</p>' : reviewQueue.map(doc => `
                        <div class="card">
                ...
                <div class="col ${role === 'ADMIN' ? '' : 'locked'}">
                    ...
                    ${adminQueue.length === 0 ? '<p>No admission apps waiting.</p>' : adminQueue.map(doc => `
                        <div class="card">
                            <h3 style="margin-top:0;">📄 ${doc.fileName}</h3>
                            <pre style="background:#f8f9fa; padding:10px; border-radius:5px; font-size:12px; white-space:pre-wrap;">${doc.content}</pre>
...
app.get('/decide', (req, res) => {
    ...
    if (req.cookies.userRole !== role) return res.status(403).send('ERROR: Unauthorized Role.');
...
app.listen(MY_PORT, () => {
    ...
    console.log(`Reviewer Key:   RADMISSION`);
    console.log(`Admin Key:      AADMISSION`);
    console.log(`==============================================\n`);
});